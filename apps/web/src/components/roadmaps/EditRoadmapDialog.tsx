...
import { useUpdateRoadmap } from '@/hooks/useRoadmaps';
...
export function EditRoadmapDialog({ roadmap, open, onOpenChange }: Props) {
  const update = useUpdateRoadmap(roadmap._id);
  const [title, setTitle] = useState(roadmap.title);
  const [description, setDescription] = useState(roadmap.description ?? '');
  const [deadline, setDeadline] = useState<Date | undefined>(
    roadmap.deadline ? new Date(roadmap.deadline) : undefined
  );
  ...
  useEffect(() => {
    if (open) {
      ...
      setDeadline(roadmap.deadline ? new Date(roadmap.deadline) : undefined);
    }
  }, [open, roadmap]);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle className="text-xl font-semibold tracking-tight">Edit roadmap</DialogTitle>
          <DialogDescription>Adjust the title, description, or deadline.</DialogDescription>
        </DialogHeader>
